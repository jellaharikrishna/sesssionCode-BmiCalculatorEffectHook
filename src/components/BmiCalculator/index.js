...
const BmiCalculator = () => {
  const sortedHeight = JSON.parse(localStorage.getItem('height'))
  const sortedWeight = JSON.parse(localStorage.getItem('weight'))

  const [height, setHeight] = useState(sortedHeight !== null ? sortedHeight : 170)
  const [weight, setWeight] = useState(sortedWeight !== null ? sortedWeight : 60)
